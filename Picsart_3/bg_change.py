...
PAYLOAD = {"format": "PNG", "output_type": "cutout"} # Output format is PNG

def change_image_background(image_full_path, bg_image_full_path, output_folder_name_at_project_root, output_file_name_in_folder):
    """
    Changes the background of an image using the Picsart API and saves the result.

    Args:
        image_full_path (str): Absolute path to the foreground image file.
        bg_image_full_path (str): Absolute path to the background image file.
        ...
        output_file_name_in_folder (str): Name for the saved output file (e.g., "Task3_result.png").
    """
    ...
    if not os.path.exists(image_full_path):
        print(f"Error: Foreground image file not found at {image_full_path}")
        return
    if not os.path.exists(bg_image_full_path):
        print(f"Error: Background image file not found at {bg_image_full_path}")
        return
    ...
    try:
        with open(image_full_path, "rb") as img_file, open(bg_image_full_path, "rb") as bg_img_file:
            files = {
                # Use original filenames for the multipart request
                "image": (os.path.basename(image_full_path), img_file, "image/png"), # Assuming PNG, adjust if necessary
                "bg_image": (os.path.basename(bg_image_full_path), bg_img_file, "image/png") # Assuming PNG
            }

            print(f"Processing {os.path.basename(image_full_path)} with background {os.path.basename(bg_image_full_path)}...")
...
def main():
    """
    Main function to configure and run the background change process.
    """
